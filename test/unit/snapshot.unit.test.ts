...
void test('rebuildSnapshot handles empty history and replays trades in order', () => {
  assert.deepEqual(rebuildSnapshot([]), { settledCash: '0', reservedCash: '0', positions: [] });
  const orders = [
  ...
  assert.deepEqual(snapshot.positions[0], { instrumentId: '1', quantity: 6, reservedQuantity: 0, cost: '300', inconsistent: false });
});
